...
urls = "https://weixin.sogou.com/weixin?type=2&query="
tail = '&_sug_type_=&sut=5699&lkt=4%2C1621414963752%2C1621414969406&s_from=input&_sug_=y&type=2&sst0=1621414969509&page='
...
def get_main_texts(soup):
    texts = soup.find_all('p', class_='txt-info')
    main_texts = [x.text.replace('\u3000' * 13, '\n\n') for x in texts]
    return main_texts


def get_new_sources(soup):
    sourcess = soup.find_all('div', class_='s-p')
 #   timess = soup.find_all('a', class_='account')
    timeAndsource=[]
    timeAndsource = soup.select('.s2')
    pub_sources = []
    pub_times = []
    for i in sourcess:
        con = i.text.strip().split('\n')
        pub_sources.append(con)
    for i in range(len(timeAndsource)):
        con = timeAndsource[i]
        pub_times.append(con)
        # print(pub_times)
   #     a = i.text.strip().split('\n')
   #      b = i.text.strip().split('\n')
   #      c = i.text.strip().split('\n')
   #      pub_sources.append(a)
   #      pub_times.append(c)
...
def get_titles(soup):
    ...
    titles = [str(i.text.strip().split('\n')) for i in Titles]
    return titles


def get_sub_urls(soup):
    sub_urls = soup.find_all('a', href=True, class_='account')
    results = [str('https://weixin.sogou.com') + str(i['href']) for i in sub_urls]
    return results

def WriteToExcel(contents):
    col = ['博物馆名称', '新闻来源', '新闻标题', '简介内容', '次级新闻网站', '新闻发布时间']
    csv = pd.DataFrame(columns=col, data=contents)
    csv.to_csv('sougou_news.xlsx')


if __name__ == '__main__':
    contents = []
    for name in NameExhibition:
        for page in range(0, 5):  # 前十页
            ...
            print(len(sub_urls))
            content = [[name, sources[i], titles[i], main_texts[i], sub_urls[i], times[i]] for i in
                       range(len(main_texts))]
